...
NrUeMac::GetNrSlTxOpportunities (const SfnSf& sfn)
{
  ...
  uint32_t mTotal = GetTotalSubCh (m_poolId)*candSsResoA.size (); // total number of candidate \s{single-slot} resources
  ...
  if (m_enableSensing)
    {
          ...
          while (itCandSsResoA != nrCandSsResoA.end ())
            {
              ...
              for (uint16_t i = 0; i < 2; i++)//m_cResel
                {
                  ...
                  for (auto &itFutureCand:listFutureCands)
                    {
                      for (const auto &itFutureSensTx:itSensedData)
                        {
                          if ((itFutureCand.sfn.Normalize () == itFutureSensTx.sfn.Normalize ()) & (itCandSsResoA->occupiedSbCh.find(itFutureSensTx.sbChStart) == itCandSsResoA->occupiedSbCh.end()))
                            {
                              ...
                              for (uint8_t i = itFutureSensTx.sbChStart; i < lastSbChInPlusOne; i++)
                                {
                                  //std::cout<<int(itFutureCand.sfn.Normalize ())<<std::endl;
                                  //std::cout<<int(i)<<std::endl;
                                  //std::cout<<itFutureSensTx.slRsrp<<std::endl;
                              ...
                              if (itCandSsResoA->occupiedSbCh.size () == GetTotalSubCh (m_poolId))//this erases slots with no available resource to make things easier later
                                {
                              //    if(itFutureSensTx.slRsrp > rsrpThrehold)
                              //      {
      ...
      while (eliminatedResources > (1 - (GetResourcePercentage () / 100.0)) * mTotal);//nrCandSsResoA.size () < (GetResourcePercentage () / 100.0) * mTotal, nrCandSsResoA.size () is the number of remaining SLOTS needs to be multiplied by NSe for comparison to X
      //std::cout<<eliminatedResources<<std::endl;
      //std::cout<<(1 - (GetResourcePercentage () / 100.0)) * mTotal<<std::endl;
      //std::cout<<int(nrCandSsResoA.size ())<<std::endl;
      //std::cout<<rsrpThrehold<<std::endl;
      std::ofstream elimResoFile;
      std::ofstream rsrpThreFile;
      if (Simulator::Now ().GetSeconds () > 5)
        {
          elimResoFile.open ("eliminatedResources.csv", std::ios_base::app);
          elimResoFile << eliminatedResources << "\n";
          elimResoFile.close();

          rsrpThreFile.open("rsrpThreshold.csv", std::ios_base::app);
