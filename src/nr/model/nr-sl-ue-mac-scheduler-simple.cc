                                            ...
                                            std::set<NrSlSlotAlloc> &slotAllocList)
{
  ...
  //what does this even determine? what is the buffersize and how does it relate to anything going on?
...
NrSlUeMacSchedulerSimple::RandomlySelectSlots (std::list <NrSlUeMacSchedSapProvider::NrSlSlotInfo> txOpps)
{
  ...
  uint8_t totalTx = GetSlMaxTxTransNumPssch ();//NSe
  ...
  for (const auto &it:txOpps)
    {
      ...
      //at UE MAC
      //no, this doesnt make any sense, the sensing has been changed and this needs to be changed with it
      if (indexes.size () == 0)
        {
          for (uint8_t i = 0; i < GetTotalSubCh(); i++)
            {
